...
import * as productCommissionRateService from './productCommissionRate.service'
...
productDiscountRouter.get("/", async (request: Request, response: Response) => {
    try {
        const data = await productCommissionRateService.list()
        if (data) {
...
productDiscountRouter.get("/:id", async (request: Request, response: Response) => {
    ...
    try {
        const data = await productCommissionRateService.get(id)
        if (data) {
            return response.status(200).json({ data: data });
        }
